import { Accordion, AccordionDetails, AccordionSummary, Box, Typography, CardMedia, List, ListItem } from '@mui/material'
    ...
    <Box sx={{ mt: 5, width: '100%', maxWidth: 640, mx: 'auto' }}>
        {
            qnAns.map((item,j) => (
                <Accordion disableGutters key={j} expanded={expanded === j} onChange={handleChange(j)}>
                    ...
                    />}>
                        <Typography sx={{width: '90%', flexShrink:0}}>{item.questionInWords}</Typography>
                    </AccordionSummary>
